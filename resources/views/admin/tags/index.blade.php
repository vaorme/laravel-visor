<x-admin-layout>
    ...
    <div class="template-1 flex flex-wrap">
                        ...
                        @foreach ($loop as $i)
                            <tr>
                                ...
                                <td>
                                    <div class="buttons">
                                        @can('tags.destroy')
                                            <a href="{{ route('tags.index') }}" class="tagsDelete" data-id="{{ $i->id }}">
                                        ...
                                        @endcan
                                        @can('tags.edit')
                                            <a href="{{ route('tags.index', ['id' => $i->id]) }}">
        ...
        <aside class="side w-2/5">
            <div class="frmo">
                    <h2>{{ isset($edit)? "Editar" : 'Crear' }} tag</h2>
                <form action="{{ isset($edit)? route('tags.update', ['id' => $edit->id]) : route('tags.store') }}" class="form{{ (isset($edit))? ' editing' : null }}" method="POST">
                    @csrf
                    @if (isset($edit))
                        @method('PATCH')
                    @endif
