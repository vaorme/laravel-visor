<x-admin-layout>
    <x-slot:title>Manga | Demografias</x-slot>
    <x-admin.nav>
    ...
    <div class="template-1 flex flex-wrap">
                    ...
                    <tbody>
                        @foreach ($loop as $i)
                            <tr>
                                ...
                                <td>
                                    <div class="buttons">
                                        @can('manga_demography.destroy')
                                            <a href="{{ route('manga_demography.index') }}" class="mangaDemographyDelete" data-id="{{ $i->id }}">
                                        ...
                                        @endcan
                                        @can('manga_demography.edit')
                                            <a href="{{ route('manga_demography.index', ['id' => $i->id]) }}">
        ...
        <aside class="side w-2/5">
            <div class="frmo">
                    <h2>{{ isset($edit)? "Editar" : 'Crear' }} demografia</h2>
                <form action="{{ isset($edit)? route('manga_demography.update', ['id' => $edit->id]) : route('manga_demography.store') }}" class="form" method="POST">
                    @csrf
                    @if (isset($edit))
                        @method('PATCH')
                    @endif
