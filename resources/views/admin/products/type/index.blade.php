    ...
    <div class="template-1 flex flex-wrap">
                    ...
                    <tbody>
                        @foreach ($loop as $i)
                            <tr>
                                ...
                                <td>
                                    <div class="buttons">
                                        @can('product_types.destroy')
                                            <a href="{{ route('product_types.index') }}" class="productTypeDelete" data-id="{{ $i->id }}">
                                        ...
                                        @endcan
                                        @can('product_types.edit')
                                            <a href="{{ route('product_types.index', ['id' => $i->id]) }}">
        ...
        <aside class="side w-2/5">
            <div class="frmo">
                ...
                <form action="{{ isset($edit)? route('product_types.update', ['id' => $edit->id]) : route('product_types.store') }}" class="form{{ (isset($edit))? ' editing' : null }}" method="POST">
                    @csrf
                    @if (isset($edit))
                        @method('PATCH')
                    @endif
