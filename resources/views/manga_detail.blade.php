	...
	<div class="main__wrap manga__detail">
        <section class="manga__card">
            <div class="manga__cover">
                <img data-src="{{ asset('storage/'.$manga->featured_image) }}" alt="{{ $manga->name }}" class="lazy"/>
                <div class="manga__terms">
                    @if ($manga->demography)
                        <div class="manga__demography {{ $manga->demography->slug }}">
                            <a href="{{ $manga->demography->url() }}">{{ $manga->demography->name }}</a>
                        </div>
                    @endif
                    @if ($manga->type)
                        <div class="manga__type {{ $manga->type->slug }}">
                            <a href="{{ $manga->type->url() }}">{{ $manga->type->name }}</a>
                        </div>
                    @endif
                </div>
            </div>
                    ...
                    @if (Auth::check())
                        <div class="manga__actions">
                            @if ($manga->userFollowManga)
                                <button class="action__follow unfollow" data-tippy-placement="top" data-tippy-content="Dejar de seguir" data-id="{{ $manga->id }}">Siguiendo</button>
                            @else
                                <button class="action__follow follow" data-id="{{ $manga->id }}">Seguir</button>
                            @endif
                            @if ($mangaViewed)
                                <button class="action__view unview" data-id="{{ $manga->id }}" data-tippy-placement="top" data-tippy-content="Desmarcar como visto">
                            ...
                            @else
                                <button class="action__view view" data-id="{{ $manga->id }}" data-tippy-placement="top" data-tippy-content="Marcar como visto">
                            ...
                            @endif
                            @if ($mangaFavorite)
                                <button class="action__fav unfav" data-id="{{ $manga->id }}" data-tippy-placement="top" data-tippy-content="Desmarcar como favorito">
                            ...
                            @else
                                <button class="action__fav fav" data-id="{{ $manga->id }}" data-tippy-placement="top" data-tippy-content="Agregar a favoritos">
                                    <svg width="18" height="10" clip-rule="evenodd" fill-rule="evenodd" stroke-linejoin="round" stroke-miterlimit="2" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                        <path d="m11.322 2.923c.126-.259.39-.423.678-.423.289 0 .552.164.678.423.974 1.998 2.65 5.44 2.65 5.44s3.811.524 6.022.829c.403.055.65.396.65.747 0 .19-.072.383-.231.536-1.61 1.538-4.382 4.191-4.382 4.191s.677 3.767 1.069 5.952c.083.462-.275.882-.742.882-.122 0-.244-.029-.355-.089-1.968-1.048-5.359-2.851-5.359-2.851s-3.391 1.803-5.359 2.851c-.111.06-.234.089-.356.089-.465 0-.825-.421-.741-.882.393-2.185 1.07-5.952 1.07-5.952s-2.773-2.653-4.382-4.191c-.16-.153-.232-.346-.232-.535 0-.352.249-.694.651-.748 2.211-.305 6.021-.829 6.021-.829s1.677-3.442 2.65-5.44z" fill-rule="nonzero"/>
                                    </svg>
                                </button>
        ...
        <section class="manga__center">
            ...
            <aside class="manga__sidebar">
                @hasrole('developer|administrador|moderador')
                <div class="sidebar__module">
                    <div class="module__content">
                        <div class="manga__actions">
                            <a href="{{ url('/controller/manga/'.$manga->id) }}" class="action__link create"><i class="fa-solid fa-plus"></i> Crear capítulo</a>
                ...
                <div class="sidebar__module">
                    ...
                    <div class="module__content">
                        <div class="manga__share">
                            <ul>
                                <li>
                                    <a href="https://www.facebook.com/sharer/sharer.php?u={{ $manga->url() }}" class="share__link fb" target="_blank" data-tippy-content="Compartir en Facebook"><i class="fa-brands fa-facebook-f"></i></a>
                                </li>
                                <li>
                                    <a href="https://twitter.com/intent/tweet?url={{ $manga->url() }}&text={{ $manga->name }}" class="share__link tw" target="_blank" data-tippy-content="Compartir en Twitter"><i class="fa-brands fa-twitter"></i></a>
                                </li>
                                <li>
                                    <a href="https://api.whatsapp.com/send?text={{ $manga->name }}+{{ $manga->url() }}" class="share__link wp" target="_blank" data-tippy-content="Compartir en Whatsapp"><i class="fa-brands fa-whatsapp"></i></a>
                                </li>
                                <li>
                                    <a href="https://telegram.me/share/url?url={{ $manga->url() }}&text={{ $manga->name }}" class="share__link tl" target="_blank" data-tippy-content="Compartir en Telegram"><i class="fa-brands fa-telegram"></i></a>
                                </li>
                                <li>
                                    <a href="http://vk.com/share.php?url={{ $manga->url() }}&title={{ $manga->name }}" class="share__link vk" target="_blank" data-tippy-content="Compartir en VK"><i class="fa-brands fa-vk"></i></a>
                                </li>
                            </ul>
                        </div>
                    </div>
